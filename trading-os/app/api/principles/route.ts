...
export async function GET() {
  ...
  const result = await db.execute(
    "SELECT * FROM principles ORDER BY CASE WHEN COALESCE(active,1)=0 THEN 1 ELSE 0 END, CASE asset WHEN 'global' THEN 1 WHEN 'XAUUSD' THEN 2 WHEN 'SOLUSD' THEN 3 ELSE 4 END, category, id"
  );
